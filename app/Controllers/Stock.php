     ...
     if($this->request->getPost('submit') != NULL OR $post_submit_check==1){
      ...
      if($this->session->get('filter') != NULL){
        ...
        if(isset($search['make']) AND !isset($search['model'])){
            $data['h1'] = ucwords(strtolower($search['make'])).' Cars For Sale In QLD With Price';
            $meta = stock_meta_make($search['make']);
        }
        elseif(isset($search['make']) AND isset($search['model'])){
            $data['h1'] = ucwords(strtolower($search['make'])).' '.ucwords(strtolower($search['model'])).' Cars For Sale In QLD';
            $meta = stock_meta_make_model($search['make'], $search['model']);
        }
        elseif(isset($search['veh_condition'])){
            $data['h1'] = 'Buy '.$search['veh_condition'].' Cars For Sale In QLD';
        ...
        elseif(isset($search['drive'])){
           ...
           $data['h1'] = 'Used Cars For Sale In QLD With Price';
                ...
                $data = array(
                    ...
                    $data = [
                     ...
                     if($this->vehInquiryModel->insert($data))
                     {
                        ...
                        $email_obj->setTo([
                            'info@autocraftqld.com',
                            'Iftikhar@sendajapan.com',
                            'mnoman55@gmail.com',
                            'peter@preferredautos.com.au',
                            'cars@preferredautos.com.au',
                            'ujawan42@gmail.com'
                        ]);
                        // $email_obj->setTo('info@autocraftqld.com');     
                        // $email_obj->setBCC(['Iftikhar@sendajapan.com', 'mnoman55@gmail.com', 'peter@preferredautos.com.au', 'cars@preferredautos.com.au']);
                        $email_obj->setReplyTo('info@autocraftqld.com', 'AUTOCRAFT QLD');
                        
                        // Add headers to improve deliverability
                        $email_obj->setHeader('X-Mailer', 'AUTOCRAFT QLD Website');
                        $email_obj->setHeader('X-Priority', '3');
                        $email_obj->setHeader('Importance', 'Normal');
                        $email_obj->setHeader('List-Unsubscribe', '<mailto:info@autocraftqld.com?subject=unsubscribe>');
